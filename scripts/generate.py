...
REFUND_REASONS = ["customer_request", "duplicate_charge", "fraud", "item_not_received"]
...
USERS_COLS = [
    "user_id", "email", "phone", "first_name", "last_name", "status",
    "created_at", "updated_at", "_source_system", "_ingested_at",
]
PRODUCTS_COLS = [
    "product_id", "name", "description", "category", "price", "currency",
    "is_active", "created_at", "updated_at", "_source_system", "_ingested_at",
]
SESSIONS_COLS = [
    "session_id", "user_id", "anonymous_id", "started_at", "ended_at",
    "duration_seconds", "device_type", "platform", "country", "referrer",
    "_source_system", "_ingested_at", "_batch_id",
]
TXNS_COLS = [
    "transaction_id", "user_id", "session_id", "product_id", "amount", "currency",
    "status", "event_at", "updated_at", "refund_amount", "refunded_at",
    "refund_reason", "_source_system", "_ingested_at",
]
...
def gen_initial_products(fake: Faker, day: int) -> list[dict]:
    ...
    for _ in range(N_PRODUCTS):
        ...
        products.append({
            ...
            "currency": random.choices(["USD", "EUR"], weights=[0.85, 0.15])[0],
            "is_active": True,
            "created_at": iso(created),
            "updated_at": iso(created),
...
def gen_daily_transactions(
    ...
    for _ in range(N_TXNS_PER_DAY):
        ...
        if eligible_sessions and random.random() > 0.05:
            ...
            event_at = datetime.fromisoformat(session["started_at"]) + timedelta(
                seconds=random.randint(60, 1500)
            )
        else:
    ...
    if day >= 2:
        refundable = [
            ...
            and (ingested_today - datetime.fromisoformat(t["event_at"])).days >= 1
        ]
        for t in random.sample(refundable, min(random.randint(1, 2), len(refundable))):
            full = random.random() > 0.3
            amount = float(t["amount"])
            refund_amount = amount if full else round(amount * random.uniform(0.2, 0.8), 2)
            t["status"] = "refunded" if full else "partially_refunded"
            t["refund_amount"] = refund_amount
            t["refunded_at"] = iso(ingested_today - timedelta(hours=random.randint(1, 12)))
...
def advance_one_day(fake: Faker, day: int) -> None:
    if day == 1:
        ...
        txns = gen_daily_transactions(fake, day, users, products, sessions_today, [])
    ...
    sessions_today = gen_daily_sessions(fake, day, users, read_csv(SESSIONS_FILE))
...
def main() -> None:
    ...
    parser.add_argument("--reset", action="store_true", help="Wipe data/ and start over at day 1.")
    parser.add_argument("--days", type=int, default=1, help="How many days to advance (default 1).")
    args = parser.parse_args()

    if args.reset:
        for p in [USERS_FILE, PRODUCTS_FILE, SESSIONS_FILE, TXNS_FILE, STATE_PATH]:
    ...
    for day in range(start_day, end_day + 1):
        ...
        print(f"  day {day} ({sim_date})  → users={sum(1 for _ in read_csv(USERS_FILE))}  "
              f"products={sum(1 for _ in read_csv(PRODUCTS_FILE))}  "
              f"sessions={sum(1 for _ in read_csv(SESSIONS_FILE))}  "
              f"txns={sum(1 for _ in read_csv(TXNS_FILE))}")
