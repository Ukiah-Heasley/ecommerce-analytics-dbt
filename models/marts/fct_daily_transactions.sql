...
with txns_to_aggregate as (
    ...
    and date_trunc('day', event_at)::date in (
        ...
        from {{ ref('fct_transactions') }}
        where _ingested_at > (
            select coalesce(max(ingested_at_high_water), timestamp '1900-01-01')
            from {{ this }}
        )
    )
    {% endif %}
...
select
    ...
    count(distinct case when canonical_user_id != '{{ var("default_unknown") }}' then canonical_user_id end) as daily_transaction_users,
