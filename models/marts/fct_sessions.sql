...
pending_resolution as (
    select session_id
    from {{ this }}
    where user_sk = {{ default_sk() }}
      and started_at >= current_date - interval '{{ var("dim_resolution_replay_days") }} days'
),
{% endif %}
sessions_to_load as (
    select *
...
select
    ...
    coalesce(sc.session_context_id, {{ default_sk(var("default_unknown") ~ '|' ~ var("default_unknown")) }})                       as session_context_id,
...
left join {{ ref('dim_session_context') }} sc
  on coalesce(s.device_type, '{{ var("default_unknown") }}') = sc.device_type
  and coalesce(s.platform,    '{{ var("default_unknown") }}') = sc.platform

left join {{ ref('dim_referrer') }} r
